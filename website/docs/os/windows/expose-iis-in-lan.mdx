...
My mac cannot access IIS server because of <TextHighlight>IIS server does not support running on macOS</TextHighlight>. And I want to use my mac to call the API on another Windows computer via LAN.
...
Then open Visual Studio as <TextHighlight>administrator</TextHighlight>.

## Configure Firewall

Open the Control Panel / System and Safe / Windows Defender Firewall. Then select advance settings and <TextHighlight>create a new inbound rule</TextHighlight>.
