...
- `shared` `Default` indicates that the page's `.wxss` style will affect custom components, and the style specified in the `.wxss` of custom components will also <TextHighlight>affect the page and other custom components configured with apply-shared or shared</TextHighlight>. (This option is not available in plugins.)
